...
* 允許使用者透過核取方塊選擇 **一張** 優惠券。若已選擇一張，選擇另一張將自動取消前一張的選中狀態。
* 前端透過 AJAX 將選中的優惠券代碼（若有）發送到後端，包含完整的錯誤處理和用戶反饋。
* 後端根據選中的 **單張** 優惠券計算折扣，並提供詳細的計算日誌。若未選擇優惠券，則不計算折扣。
* 後端返回計算結果 (原始總價、折扣後總價、總折扣、套用的優惠券列表（最多一個）)。
...
* **可部署性 (Deployability)**：專案應易於在本機環境中運行和部署。
...
**範圍內 (In Scope):**
詳細功能請參見 [1.2.2 主要功能需求](#122-主要功能需求)。核心功能包括商品展示、優惠券選擇、價格計算（錯誤邏輯）、結果呈現。
