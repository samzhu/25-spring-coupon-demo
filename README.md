...
此應用程式的核心目標是：
* **優惠券疊加使用邏輯呈現**：清晰地呈現當業務規則定義優惠券可以疊加使用時的計算結果與系統行為。
* **功能完整性**：提供一個可操作的購物車界面，允許用戶查看固定的商品、透過勾選選擇多張優惠券，並即時看到基於業務邏輯計算出的價格。
* **設計啟示**：幫助開發者理解在設計優惠券系統時，如何實現並管理優惠券的疊加使用規則。
* **技術實現**：一個使用特定技術棧（Java、Spring Boot、Mustache、jQuery、AJAX）實現的 Web 應用程式。

> **動機**: 從終端使用者的角度來看，此系統的建立旨在提供一個清晰的參考，說明標準電商情境下優惠券疊加使用的軟體行為，並輔助技術理解。

#### 1.2.2 主要功能需求 (Key Functional Requirements)

> **內容**: 系統需提供基本的購物車操作界面，並能根據選定的多張優惠券計算價格。

* 展示固定的購物車商品列表（包含 4 個預設產品）。
* 展示所有可用的固定金額優惠券列表（包含 4 個預設優惠券）。
* 允許使用者透過核取方塊勾選多張優惠券。
* 前端透過 AJAX 將所有選中的優惠券代碼列表發送到後端進行計算。
* 後端根據所有選中的有效優惠券計算總折扣，返回計算結果 (原始總價、折扣後總價、總折扣、套用的優惠券列表)。
* 前端動態更新價格和已套用優惠券列表（可能有多個），並在發生錯誤時顯示適當的錯誤訊息。
* 允許使用者清除所有已選的優惠券。
...
5.  **`CartService`**: `calculateTotalsFromDto` 方法執行：
    ...
    * 檢查 `shoppingCartInput.couponCodes`：
        ...
        * 如果列表包含一個或多個優惠券代碼，則遍歷此列表：
            * 對於每個優惠券代碼，從 `CouponRepository` 獲取 `Coupon` 物件。若優惠券不存在或無效，則拋出例如 `InvalidCouponException` 的業務例外 (或根據策略選擇忽略無效券並繼續處理有效券，本範例假設遇到無效券即中止並報錯)。
            * 將有效的 `Coupon` 物件加入到結果列表，並累加其 `discountAmount` 到總折扣金額。
        * 記錄詳細的計算過程日誌。
        * **計算邏輯**：總折扣金額為所有有效且被選中優惠券的 `discountAmount` 之和。
    * 計算折扣後總價 (原始總價 - 總折扣金額)，確保不會出現負數。若計算結果不合規，可拋出相應業務例外。
    * 若一切正常，返回一個包含計算結果的 `CalculationResultDto` (其中 `appliedCoupons` 列表可能包含多個優惠券)。
6.  **`CartController`**: 
...
8.  **瀏覽器 (jQuery - `renderCalculationResult` 函數)**:
    * 使用返回的數據更新頁面上的相應元素（原始總價、折扣後總價、已套用優惠券列表 - 現在可能需要展示多個優惠券、總節省金額等）。
...
* **AD6: 優惠券疊加使用策略**
    * **決策**: 系統設計為允許使用者在一次交易中選擇並套用多張有效的優惠券。所有被選中且有效的優惠券折扣將被疊加計算，以產生最終的總折扣金額。
    * **理由**:
        * **提升促銷靈活性**: 允許多張優惠券疊加，可以為行銷活動提供更大的彈性和吸引力，例如允許使用者同時使用「滿額減」和「特定商品折扣券」。
        * **滿足多樣化行銷場景**: 某些業務情境下，疊加使用優惠券是合理的，例如，鼓勵用戶購買更多商品或嘗試新產品。
        * **簡化初始演示邏輯**: 在此範例專案中，為清晰展示多券疊加的計算過程，暫不引入複雜的優惠券互斥規則（例如，哪些券之間不能疊加）。所有可用的、被選中的有效優惠券都將直接疊加其折扣金額。
        * **潛在的顧客激勵**: 允許疊加可能增加顧客的購買意願和客單價。
    * **考量與未來擴展**:
        * **風險控制**: 在實際生產環境中，無限制的疊加可能導致過度折扣或財務風險。通常需要配合更精細的規則引擎來定義哪些優惠券可以疊加、疊加順序、以及疊加的上限等。
        * **複雜性增加**: 相較於擇一使用，疊加邏輯會增加後端計算的複雜性，以及前端展示多張已套用優惠券的複雜度。
        * **替代方案 (此專案修改前的設計)**:
            * **優惠券排他性使用**: 即使用者一次只能選擇並套用一張優惠券。此方案邏輯簡單，易於實現和管理，能有效控制折扣成本，但促銷靈活性較低。對於此專案的新目標（演示疊加邏輯），此方案不再適用。
            * **有條件的疊加**: 例如，允許特定類型的優惠券疊加，或達到一定消費門檻後允許疊加。這會增加業務規則和系統實現的複雜度，但比無限制疊加更可控。
* **AD7: DTO 用於服務層接口**
